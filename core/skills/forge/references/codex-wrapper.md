...
Codes 65 (`CODEX_DIFF_INVALID`), 69 (`CODEX_NOT_LOGGED_IN`; run `codex login` for that `CODEX_HOME`), 76 (`CODEX_BUDGET_EXCEEDED`), 77 (`CODEX_NO_CREDITS`), and 78 (`CODEX_LOCK_TIMEOUT`) are terminal. Report their result-file message and do not retry them. Exit 79 is handled only by the fresh handoff loop above. A wrapper retry requested by the workflow must run the supplied command again; never answer from a previous cached result.
