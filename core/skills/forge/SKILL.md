...
3. Review every build plan once. Run `scripts/build-plan-review-prompt.sh`, which accepts either `recon.md` or `triage.md`, and write any extra file-and-line claims to `plan-review-checks.md`. The script caps the inline excerpt pack at 60 KB by default; pass `--max-excerpt-kb N` to change it. Use `roles.plan-review` from `forge.config.json`: invoke `codex-exec.sh --role plan-review` for Codex or the configured Claude reviewer with the same prompt. The reviewer prints the review and writes no file; the caller saves the printed review to `<runDir>/plan-review.md` (for Codex, its final message). Fold critical findings into the plan and record rejected findings with reasons.
4. Publish the plan artifact: the main session writes `<runDir>/plan-data.json` (keys in the header comment of `references/plan-artifact.html`, including `decision`: what is being approved, the size, and the top risk); a worker runs `python3 ~/.claude/skills/forge/scripts/render_artifact.py plan --data <runDir>/plan-data.json --markdown <runDir>/plan.md --out <runDir>/plan-artifact.html`; the main session then publishes that file with the Artifact tool, which sub-agents do not have. Without an Artifact tool, open the local HTML file instead. Get explicit user confirmation of the plan unless `auto` was requested. In every mode, present the test table on its own, before or after the plan summary, and ask for an explicit yes on the tests as a separate answer from plan approval. For a no-tests plan, present `None: <reason>` in place of the table and get the same separate approval. A plan whose tests are not approved does not start; an `auto` run can proceed unattended only after that approval.
5. Load the rendered Forge config and start the Workflow. For multi-repository work, pass the repository-specific `planPath`. Set `fullySpecified=true` only when the caller supplied every material implementation choice; file count alone never makes work fully specified. The `runDir` path must not contain a word the secret-read hook blocks in Write paths (`token`, `secret`, `credential`, `passwd`, `shadow`, `api-key` and their plurals); forge writes its state files with the Write tool and stops at start when it does. Before starting, write `<runDir>/launch-args.json` with the Write tool: the args exactly as launched minus `planText`, with `planPath`. Keep `forgeConfig` in it when it was passed inline; a successor session cannot rebuild inline overrides such as stages turned off. The Workflow tool does not expand `~`, so pass `scriptPath` as an absolute path.
